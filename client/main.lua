...
local function MenuGarage()
    ...
    lib.showContext('open_towGarage')
end
...
local function CreateZone(type, number)
    ...
    if type == "main" then
        ...
        coords = vec3(Config.Locations[type].coords.x, Config.Locations[type].coords.y, Config.Locations[type].coords.z)
    ...
    elseif type == "towspots" then
        ...
        coords = vec3(Config.Locations[type][number].coords.x, Config.Locations[type][number].coords.y, Config.Locations[type][number].coords.z)
...
RegisterNetEvent('qb-tow:client:Vehicle', function()
    ...
    if not CurrentTow then
        if vehicle and isTowVehicle(vehicle) then
            DeleteVehicle(GetVehiclePedIsIn(cache.ped))
